...
def start_app(app):
    ...
    cmd_rm_run_file1 = "rm -rf " + app_path + "/work/*"
    cmd_rm_run_file2 = "rm -rf " + app_path + "/webapps/*"
    os.system(cmd_rm_run_file1)
    os.system(cmd_rm_run_file2)
    app_exec = app_path + "/bin/startup.sh"
    os.system(app_exec);
    if count == 1:	
        Log=raw_input("syslog/log:")
        if Log == "syslog":
	    show_log(app,"sys")
        if Log == "log":
	    show_log(app,"app")
